...
export default () => {
  bus.on(AppEvent.JOB_V1_CAPTCHA_ERROR, async (result, job: DKHPTDJobV1) => {
    ...
    logger.info(`Retry job v1 ${job._id} because of captchaError`);
    await mongoConnectionPool.getClient()
      .db(cfg.DATABASE_NAME)
      .collection(CollectionName.DKHPTDV1)
      .updateOne({ _id: job._id }, { $set: { status: JobStatus.READY, timeToStart: Date.now() + ms("15s") } }); // set READY and delay for scheduler retry it
  });
};